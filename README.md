...
1. Récupère sur GIT le fichier contenant la conf globale des toggles attendue pour la version passée en
   paramètre ([exemple ici](conf-toggle/-/blob/master/test_ok.json))    
...
[
  {
    "projet": "comp-bff", => le nom du projet à aller chercher dans git (pas utilisé si spring-cloud-conf)
    "tag": "1.12.2", => le tag à vérifier pour le projet (pas utilisé si spring-cloud-conf), si tag = develop alors projet ignoré
    "spring-cloud-conf": "true", => clé optionnelle qui indique si la conf se trouve dans spring-cloud-conf (sinon on va chercher dans le projet lui même)
    "toggles": [ => liste des toggles
      {
        ...
        "use-refm-for-marques": [
          {
            "d": { => dans le cas dun toggle avec une strategy, on la définit de cette manière, strategy que si toggle dans spring-cloud-conf
              "value": "true",
...
Par exemple pour le comp-bff on obtient :
```json
{
  "comp-bff": {
    "d": {
...
2 types de fichiers sont traités, les fichiers configmap et les fichiers "spring-cloud-conf"
...
CONF_TOGGLES_USE_comp_SVC_DOCUMENT=true
...
#### Fichier spring-cloud-conf
Les fichiers présents dans spring-cloud-conf (abus de langage ...) sont eux en `.yml` et peuvent contenir une strategyPattern

```yaml

oyster:
  toggles:
    - name: marque-mamarque-enabled
      active: false
...
ce type de fichier est traité dans `verifier.py#search_toggle_in_spring-cloud-conf_files`.


### Fichier retour
Pour chaque projet, on vient chercher dans les fichiers `configmap.properties` ou dans `spring-cloud-conf` les toggles.   
