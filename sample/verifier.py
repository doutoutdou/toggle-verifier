...
from gitlab_helper import get_project_id, get_project_configuration, get_spring_cloud_conf_configuration, get_toggles
import yaml
...
def search_toggle_in_configuration_files(environment_key, project_id, tag_version, toggle_dict_by_env):
    ...
    for toggle_name, toggle_configuration in list(toggle_list_by_env.items()):
        ...
        # Pas de strategy pour un toggle qui n'est pas dans spring_cloud_conf
...
# Effectue la recherche des toggles dans spring_cloud_conf
def search_toggle_in_spring_cloud_conf_files(environment_key, tag_version, toggle_dict_by_env):
    toggle_list_by_env = toggle_dict_by_env[environment_key]
    # On récupère la conf des toggles sur GIT pour un projet & 1 envir donné
    project_configuration = yaml.load(get_spring_cloud_conf_configuration(environment_key, tag_version))
    toggles_from_spring_cloud_conf = project_configuration["oyster"]["toggles"]
    for toggle_name, toggle_configuration in list(toggle_list_by_env.items()):
        ...
        # Performance \o/
        for spring_cloud_conf_toggle in toggles_from_spring_cloud_conf:
            # On vérifie si le toggle du fichier spring_cloud_conf correspond au toggle attendu
            if spring_cloud_conf_toggle["name"].casefold() == toggle_name.casefold():
                ...
                # le toggle provenant de spring_cloud_conf est par contre de la forme suivante
                # {'name': 'use-refm-for-marques', 'active': True, 'strategyParams': {'values': 'RENAULT,DACIA,GANAS,GEV,GPAT,GPREV,GPMA,NAT'}}
                if spring_cloud_conf_toggle["active"] == util.strtobool(toggle_configuration['value']):
                    ...
                    if "strategyParams" not in toggle_configuration:
                        ...
                        if "values" in spring_cloud_conf_toggle["strategyParams"] and spring_cloud_conf_toggle["strategyParams"]["values"].casefold() == toggle_configuration["strategyParams"].casefold():
                            del toggle_dict_by_env[environment_key][toggle_name]
        # si plus de toggle pour un environnement, alors on supprime l'environnement
    if len(toggle_dict_by_env[environment_key]) == 0:
        del toggle_dict_by_env[environment_key]
